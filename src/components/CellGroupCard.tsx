...
function CellGroupCardImpl({ cellGroup, showActions = false, onEdit, onDelete }: CellGroupCardProps) {
          ...
          <div className="flex flex-col gap-1 ml-2">
            {onEdit && (
              <button
                ...
                title="Edit"
                aria-label={`Edit ${cellGroup.name}`}
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {onDelete && (
              <button
                ...
                title="Delete"
                aria-label={`Delete ${cellGroup.name}`}
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export const CellGroupCard = memo(CellGroupCardImpl)
...
function CompactCellGroupCardImpl({ cellGroup, onClick }: CompactCellGroupCardProps) {
  const dayColors: Record<string, string> = {
    Sunday: 'bg-purple-100 text-purple-800',
    Monday: 'bg-blue-100 text-blue-800',
    Tuesday: 'bg-green-100 text-green-800',
...
export const CompactCellGroupCard = memo(CompactCellGroupCardImpl)
